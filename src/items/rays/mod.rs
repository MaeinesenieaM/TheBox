...
pub fn start(display: &mut BoxDisplay, sdl_context: &mut SdlContext, write: &Write) {
    ...
    let mut modifiers_label: Label = Label::new(0.0, 0.0, 8, write, None);
    ...
    'repeat: loop {
        ...
        let (mut pos_end_x, mut pos_end_y): (f32, f32) = ray_corner(
            (arrow.x, arrow.y),
        ...
        for _ in 0..ray_cycles {
            let (temp_x, temp_y): (f32, f32) = ray_corner(
                (pos_end_x, pos_end_y),
                (window_width, window_height),
                &mut next_angle,
                ray_precision,
        ...
        for modifier in modifiers.iter() {
            modifiers_label.set_pos(modifier.slider.x + 80.0, modifier.slider.y - 4.0);
            modifiers_label.update_text(modifier.slider.get_value_ref().to_string());
            let _ = modifiers_label.draw(display);

            modifiers_label.set_pos(modifier.slider.x, modifier.slider.y - 22.0);
...
fn angle_pos(x: f32, y: f32, mut angle: f32, distance: f32) -> (f32, f32) {
    angle *= PI;
    (
        x + (distance * angle.sin()),
        y + (distance * angle.cos()),
    )
}

fn rounder(value: &mut f32, max: u32) {
    if *value > max as f32 {
        *value = max as f32
    } else if *value < 0f32 {
        *value = 0f32
...
fn ray_corner(
    ...
    for i in 1..(precision + 1) {
        ...
        if px > width as f32 || px < 0.0 {
            rounder(&mut px, width);
            *angle *= -1.0;
            return (px, py);
        } else if py > height as f32 || py < 0.0 {
